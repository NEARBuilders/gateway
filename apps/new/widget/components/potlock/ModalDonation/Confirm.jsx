const { Button, User } = VM.require("${alias_old}/widget/components") || {
  Button: () => <></>,
  User: () => <></>,
};
...
const { nearToUsd } = VM.require("${alias_potlock}/widget/utils");
...
const CheckBox = ({ id, checked, onClick }) => (
  <Widget
    src={"${alias_potlock}/widget/Inputs.Checkbox"}
    props={{
      id,
      checked,
      onClick,
  ...
  return (
    <Container>
      ...
      <div>
        <Widget
          src={
            "${config_account}/widget/components.potlock.ModalDonation.BreakdownSummary"
          }
          props={{
            ...props,
            referrerId,
      ...
      <FeesRemoval>
        <div className="check">
          ...
          <User accountId={protocolFeeRecipientAccount} variant={"mobile"} />
        </div>
        {potDetail?.chef && chefFeeBasisPoints > 0 && (
          <div className="check">
            ...
            <a
              ...
              target="_blank"
            >
              <User accountId={potDetail?.chef} variant={"mobile"} />
            </a>
          </div>
        )}
      </FeesRemoval>
